...
import cassandra.tests.repositories.HistoryAIMinuteRepository;
import cassandra.tests.util.AppContextLoader;

public class HistoryAIMinuteRepositorySelectByIdAndTsRangeTest {
	int count = 0;
	...
	public void setUp() {
		...
		repository = ctxLoader.getContext().getBean(HistoryAIMinuteRepository.class);
		
		Insert insert = QueryBuilder.insertInto("history_ai_minute")
		.value("id", 1)
	...
	private HistoryAIMinuteRepository repository;
